Stage 4 Backend Task: Microservices & Message QueuesTask Title: Distributed Notification System
Goal/Objective:
Build a notification system that sends emails and push notifications using separate microservices. Each service should communicate asynchronously through a message queue (e.g., RabbitMQ or Kafka).Task Execution
...
You can use Draw.io, Miro, or Lucidchart (free tools).Performance Targets
...
:white_check_mark: Sample Request formatsPOST /api/v1/notifications/
...
May the road rise up to meet you (edited)
